...
const BreadcrumbSet: React.FC<IBreadcrumbSetProps> = props => {
    const suiSections = [homeBreadcrumb];
    props.sections.forEach(section => {
        if (section.href) {
            suiSections.push(<Breadcrumb.Section key={section.href} as={Link}
                                                 to={section.href}>{section.name}</Breadcrumb.Section>);
        } else {
